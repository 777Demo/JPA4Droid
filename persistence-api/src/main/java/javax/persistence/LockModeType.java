 ...
 * <code>LockModeType.OPTIMISTIC</code> on a versioned object, the entity manager must ensure that neither of the following phenomena can
 * occur:
 * <ul>
 * <li>P1 (Dirty read): Transaction T1 modifies a row. Another transaction T2 then reads that row and obtains the modified value, before T1
 * has committed or rolled back. Transaction T2 eventually commits successfully; it does not matter whether T1 commits or rolls back and
 * whether it does so before or after T2 commits.
 * <li>
 * </li> P2 (Non-repeatable read): Transaction T1 reads a row. Another transaction T2 then modifies or deletes that row, before T1 has
 * committed. Both transactions eventually commit successfully. </li>
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * In addition, calling a lock of type <code>LockModeType.OPTIMISTIC_FORCE_INCREMENT</code> on a versioned object, will also force an update
 ...
 * 
 * <p>
 * If transaction T1 calls for a lock of type <code>LockModeType.PESSIMISTIC_READ</code> or <code>LockModeType.PESSIMISTIC_WRITE</code> on
 ...
 * 
 * <p>
 * A lock with <code>LockModeType.PESSIMISTIC_WRITE</code> can be obtained on an entity instance to force serialization among transactions
 * attempting to update the entity data. A lock with <code>LockModeType.PESSIMISTIC_READ</code> can be used to query data using
 ...
 * transactions reading the data. A lock with <code>LockModeType.PESSIMISTIC_WRITE</code> can be used when querying data and there is a high
 * likelihood of deadlock or update failure among concurrent updating transactions.
 * 
 * <p>
 * The persistence implementation must support use of locks of type <code>LockModeType.PESSIMISTIC_READ</code>
 * <code>LockModeType.PESSIMISTIC_WRITE</code> on a non-versioned entity as well as on a versioned entity.
 * 
 * <p>
 ...
 * {@link PessimisticLockException} and ensure that the JTA transaction or <code>EntityTransaction</code> has been marked for rollback.
 * 
 * <p>
 ...
 * {@link LockTimeoutException} (and must not mark the transaction for rollback).
 * 
...
public enum LockModeType {
	...
	/**
	 * Synonymous with <code>OPTIMISTIC_FORCE_INCREMENT</code>. <code>OPTIMISTIC_FORCE_IMCREMENT</code> is to be preferred for new
	 * applications.
	 * 
	 */
	WRITE,
